...
void Sprite::shoot()
{
    // Save ship's current position so the bullet will have its own position
    if (firstShoot) {
        bulletXPos = x;
        bulletYPos = y;
        firstShoot = false;
    }
    if (++bulletCurrCell < LIMX) { // bullet on screen
        SDL_SetRenderDrawColor(_gRenderer, 255, 255, 0, 150);
        SDL_Rect renderQuad;
        renderQuad = {(bulletXPos + bulletCurrCell) * PASSO, bulletYPos * PASSO, _Width, _Height};
        SDL_RenderFillRect(_gRenderer, &renderQuad);
    }
    else { // bullet off screen
        shooting = false;
        bulletCurrCell = 0;
    }
}
